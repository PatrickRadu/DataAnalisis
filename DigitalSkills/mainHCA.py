...
try:
    ...
    fileName = './dataIN/DigitalSkillLevelCleaned.csv'
    ...
    if discriminant_axes:
        ...
        graphics.plot_clusters(z[:, 0], z[:, 1], g_max, groups,
                        labels=table['Country'].values, title='Optimal partition')
    if histograms:
        for v in vars:
    ...
    t_partitions.to_csv('./dataOUT/outHCA/Partitions.csv')
